...
let cachedMergedSportsChannels = null;
let lastFetch = 0;
let lastGuidesFetch = 0;
let lastSportsMerge = 0;
...
export const getPopularityScore = (name) => {
...
export const getSportsChannels = async (limit = 500, query) => {
    ...
    if (!cachedMergedSportsChannels || (Date.now() - lastSportsMerge > CACHE_DURATION)) {
        ...
        const filteredChannels = channels?.filter((channel) => {
            ...
            const isSportsCat = channel.categories && channel.categories.some((cat) => cat.toLowerCase() === 'sports');
...
//# sourceMappingURL=iptvApiService.js.map