...
export const syncEpgForChannel = async (channelId, tvgId) => {
    try {
        ...
        const guide = guides.find((g) => g.channel === tvgId);
        if (!guide) {
        ...
        const channelProgrammes = programmes.filter((p) => p.$ && p.$.channel === tvgId);
        const programData = channelProgrammes.map((p) => {
...
export const syncAllEpg = async () => {
    try {
        ...
        for (const channel of channels) {
            const guide = guides.find((g) => g.channel === channel.tvgId);
            if (guide && guide.url) {
        ...
        for (const [guideUrl, batch] of urlToChannels.entries()) {
            try {
                ...
                for (const channel of batch) {
                    const channelProgrammes = programmes.filter((p) => p.$ && p.$.channel === channel.tvgId);
...
//# sourceMappingURL=epgService.js.map